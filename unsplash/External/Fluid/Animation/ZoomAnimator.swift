...
class ZoomAnimator: NSObject {
    ...
    private func calculateZoomInImageFrame(image: UIImage, forView view: UIView) -> CGRect {
        ...
        let maxHeight = view.frame.height - (top + bottom)     // FluidPhoto - PhotoPageViewController top, bottom margin
        
        let viewRatio = view.frame.size.width / maxHeight
        let imageRatio = image.size.width / image.size.height
        let touchesSides = (imageRatio > viewRatio)
        
        if touchesSides {
            let height = view.frame.width / imageRatio
            let yPoint = (maxHeight - height) / 2 + safeArea.top
            return CGRect(x: 0, y: yPoint, width: view.frame.width, height: height)
        }
        else {
            let width = maxHeight * imageRatio
            let xPoint = view.frame.minX + (view.frame.width - width) / 2
            return CGRect(x: xPoint, y: top, width: width, height: maxHeight)
        }
    }
}
